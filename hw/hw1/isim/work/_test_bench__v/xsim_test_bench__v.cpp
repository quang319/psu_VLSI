...
va_list vap)
{
    extern HSim__s6 * createworkM_test_bench__v(const char*);
    HSim__s6 *blk = createworkM_test_bench__v(label); 
    return blk;
}
...
va_list vap)
{
    extern HSim__s6 * createworkM_t_r_a_f_f_i_c___l_i_g_h_t(const char*);
    HSim__s6 *blk = createworkM_t_r_a_f_f_i_c___l_i_g_h_t(label); 
    return blk;
}
...
public:
    ...
    HSimConfigDecl * topModuleInstantiate() {
        ...
        (*cfgvh).addVlogModule("work","TestBench_v", (HSimInstFactoryPtr)IF0);
        (*cfgvh).addVlogModule("work","TRAFFIC_LIGHT", (HSimInstFactoryPtr)IF1);
        (*cfgvh).addVlogModule("work","glbl", (HSimInstFactoryPtr)IF2);
        HSim__s5 * topvl = 0;
        extern HSim__s6 * createworkM_test_bench__v(const char*);
        topvl = (HSim__s5*)createworkM_test_bench__v("TestBench_v");
